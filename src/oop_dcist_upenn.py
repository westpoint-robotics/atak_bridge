...
#        self.map_img_frame      = rospy.get_param('~img_frame', 'map_image')        
        ...
        self.zone                = rospy.get_param('~zone', '18T')
        self.takmsg_tree = ''
        self.count = 1
...
#        self.uav_pub = rospy.Publisher(self.goal_topic, PoseStamped, queue_size=10)
        ...
        rospy.Subscriber('/' + self.robot_name + '/det_odom', Odometry, self.object_location_cb)        
    ...
    def object_location_cb_prev(self, data):
        for detection in data.detections:
            for result in detection.results:
                if result.id in self.target_list:
                    obj_pose_stamped = Odometry()
    ...
    def object_location_cb(self, data):
        ...
        obj_pose_stamped.pose = data.pose.pose
        obj_pose_utm = self.tf1_listener.transformPose('utm', obj_pose_stamped)
        (obj_latitude, obj_longitude) = UTMtoLL(23, obj_pose_utm.pose.position.y, obj_pose_utm.pose.position.x, self.zone) # 23 is WGS-84.
#        rospy.loginfo('+++++++++++++++')
#        rospy.loginfo(type(str(obj_pose_stamped.header.seq)))
        
#        self.count = self.count+1
#        countt = 1
        self.takserver.send(mkcot.mkcot(
						cot_identity = 'neutral',
						cot_stale    = 5, # displaying duration
						cot_type     = 'a-f-G-M-F-Q',
						cot_how      = 'm-g',
						cot_callsign = 'obj_'+self.robot_name+'_'+str(data.header.seq), # label on ATAK screen 
						cot_id       = self.robot_name+str(data.header.seq), # To display multiple detected objects
						team_name    = 'detector',
						team_role    = 'obj detector',
						cot_lat      = obj_latitude,
						cot_lon      = obj_longitude ))
...
#    def parse_takmsg_air(self):
    ...
    def robot_pose_to_tak(self):
        ...
        crnt_pose = self.tf1_listener.lookupTransform('utm', self.baselink_frame, rospy.Time(0))#-----------odom receiving
